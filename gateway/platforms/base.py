...
# ── Outbound content-fingerprint dedup gate (WO-D-2026-09-14-019-04 B) ──────────────────
# One process-local, thread-safe registry consulted by every outbound text-send funnel
# (``BasePlatformAdapter._send_with_retry`` for gateway-internal delivery and the
# ``send_message`` tool's adapter path). It is the single point that catches identical-content
# races between in-turn tool sends and the auto-delivered final response: whichever send
# arrives second within the TTL for the same (platform, chat, thread) target is dropped.
OUTBOUND_DEDUP_TTL_SECONDS = 120.0
_OUTBOUND_DEDUP_MAX_ENTRIES = 500


def normalize_outbound_content(content: Any) -> str:
    """Canonical text for duplicate comparison: CRLF/CR folded to LF, per-line trailing
    whitespace and blank leading/trailing lines stripped. The duplicate paths observed in the
    2026-09-14 incident differed only by one trailing newline (``send -f`` appends one, the
    mirror round-trip strips it), so trailing-newline-only differences MUST dedup."""
...
class _OutboundDedupGate:
    """TTL registry of recently delivered content fingerprints per (platform, chat, thread).

    State lives in a small JSON file under the profile's ``cache/`` so one gate covers BOTH
    processes that run the send funnels: the gateway (final responses, in-process tool calls)
    and the ``hermes send`` CLI subprocess — the WO-D-2026-09-14-019-04 B "single point"
    requirement (the incident's duplicate pair spanned two processes). Fingerprints are
    recorded only AFTER a successful delivery (a failed send must not block a legitimate
    retry), checked before sending. If the store is unusable the gate degrades to per-process
    memory (fail-open for delivery, dedup coverage shrinks to one process)."""

    _FORMAT_VERSION = 1

    def __init__(self, ttl_seconds: float = OUTBOUND_DEDUP_TTL_SECONDS,
                 max_entries: int = _OUTBOUND_DEDUP_MAX_ENTRIES, store_path: Any = None):
        self._ttl = float(ttl_seconds)
        self._max_entries = max(1, int(max_entries))
        self._lock = threading.Lock()
        self._store_path = Path(store_path) if store_path is not None else self._default_store_path()
        # memory fallback used only when the file store is unusable (per-process coverage).
        self._fallback: "OrderedDict[str, tuple[str, float]]" = OrderedDict()
        self._file_usable = True
    ...
    @staticmethod
    def _key(platform: Any, chat_id: Any, thread_id: Any) -> str:
        return "|".join((
    ...
    # ── file store helpers (best-effort; every failure degrades to the memory fallback) ──

    def _load(self) -> "OrderedDict[str, tuple[str, float]]":
        import json
            ...
            if isinstance(raw, dict):
                for key, pair in raw.items():
                    if isinstance(pair, (list, tuple)) and len(pair) == 2:
                        entries[str(key)] = (str(pair[0]), float(pair[1]))
            self._file_usable = True
        except FileNotFoundError:
            self._file_usable = True
        except Exception:
            self._file_usable = False
        return entries

    def _save(self, entries: "OrderedDict[str, tuple[str, float]]") -> bool:
        import json
        try:
            self._store_path.parent.mkdir(parents=True, exist_ok=True)
            tmp = self._store_path.with_name(self._store_path.name + ".tmp")
            with open(tmp, "w", encoding="utf-8") as fh:
                fh.write(json.dumps({"v": self._FORMAT_VERSION, "entries": {k: [fp, ts] for k, (fp, ts) in entries.items()}},
                                    ensure_ascii=False))
    ...
    @staticmethod
    def _prune(entries: "OrderedDict[str, tuple[str, float]]", now: float, ttl: float, cap: int) -> None:
        cutoff = now - ttl
        stale = [k for k, (_, ts) in entries.items() if ts < cutoff]
        for k in stale:
            entries.pop(k, None)
        while len(entries) > cap:
            entries.popitem(last=False)

    def check(self, platform: Any, chat_id: Any, thread_id: Any, content: Any) -> bool:
        """True when identical normalized content was already delivered to this target within the TTL."""
        fingerprint = normalize_outbound_content(content)
        if not fingerprint:
            return False
        key = self._key(platform, chat_id, thread_id)
        now = time.time()
        with self._lock, _dedup_file_lock(self._store_path):
            if not self._file_usable:
                self._prune(self._fallback, now, self._ttl, self._max_entries)
                entry = self._fallback.get(key)
                return bool(entry and entry[0] == fingerprint and now - entry[1] < self._ttl)
            entries = self._load()
            if not self._file_usable:  # load failed — degrade from now on
                self._prune(self._fallback, now, self._ttl, self._max_entries)
                entry = self._fallback.get(key)
                return bool(entry and entry[0] == fingerprint and now - entry[1] < self._ttl)
            self._prune(entries, now, self._ttl, self._max_entries)
            entry = entries.get(key)
            return bool(entry and entry[0] == fingerprint and now - entry[1] < self._ttl)

    def record(self, platform: Any, chat_id: Any, thread_id: Any, content: Any) -> None:
        """Record a successfully delivered content fingerprint (call only AFTER delivery success)."""
        fingerprint = normalize_outbound_content(content)
        if not fingerprint:
            return
        key = self._key(platform, chat_id, thread_id)
        ...
        with self._lock, _dedup_file_lock(self._store_path):
            if not self._file_usable:
                self._fallback[key] = (fingerprint, now)
            ...
            if not self._file_usable:
                ...
                self._prune(self._fallback, now, self._ttl, self._max_entries)
                return
            entries[key] = (fingerprint, now)
            entries.move_to_end(key)
            self._prune(entries, now, self._ttl, self._max_entries)
            if not self._save(entries):
                self._fallback[key] = (fingerprint, now)

    def reset(self) -> None:
        """Clear all state (tests): store file removed, memory fallback cleared."""
        with self._lock, _dedup_file_lock(self._store_path):
...
def _dedup_file_lock(store_path: Any):
    """Cross-process advisory lock around the dedup store file (POSIX flock; no-op elsewhere)."""
    try:
        import fcntl
    ...
    try:
        ...
        # lock file unusable: proceed without the cross-process guard (dedup still best-effort)
        yield


_outbound_dedup_gate = _OutboundDedupGate()


def outbound_duplicate_check(platform: Any, chat_id: Any, thread_id: Any, content: Any) -> bool:
    """True when this exact content was just delivered to the same target (should be skipped)."""
    return _outbound_dedup_gate.check(platform, chat_id, thread_id, content)


def outbound_duplicate_record(platform: Any, chat_id: Any, thread_id: Any, content: Any) -> None:
    """Record a successful delivery for dedup (WO-D-2026-09-14-019-04 B)."""
    _outbound_dedup_gate.record(platform, chat_id, thread_id, content)


def reset_outbound_dedup_gate() -> None:
    """Test/helper hook: clear the process-wide outbound dedup state."""
    _outbound_dedup_gate.reset()
...
class BasePlatformAdapter(ABC):
    ...
    async def _send_with_retry(
        ...
        # Outbound content-fingerprint dedup (WO-D-2026-09-14-019-04 B): an identical send to the
        # same (platform, chat, thread) within the TTL is the classic duplicate-content accident
        # (in-turn tool send + auto-delivered final response); drop the second one.
        _dedup_thread = (metadata.get("thread_id") if isinstance(metadata, dict) else None) or reply_to
        if outbound_duplicate_check(self.platform, chat_id, _dedup_thread, content):
            logger.info(
                "[%s] Outbound send skipped: duplicate content for chat=%s thread=%s within %.0fs TTL.",
                self.name, chat_id, _dedup_thread or "-", OUTBOUND_DEDUP_TTL_SECONDS)
            return SendResult(success=True, raw_response={"skipped": "duplicate_content"})

        async def _send(text: str) -> "SendResult":
            _attempt = await self.send(chat_id=chat_id, content=text, reply_to=reply_to, metadata=metadata)
            if getattr(_attempt, "success", False):
                outbound_duplicate_record(self.platform, chat_id, _dedup_thread, text)
            return _attempt
